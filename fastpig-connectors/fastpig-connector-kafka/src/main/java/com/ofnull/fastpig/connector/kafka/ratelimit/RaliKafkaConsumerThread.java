 ...
 */
@Internal
public class RaliKafkaConsumerThread<T> extends Thread {
    ...
    public RaliKafkaConsumerThread(
            Logger log,
            Handover handover,
            ...
            PartitionRateLimiter rateLimiter) {
        ...
        this.rateLimiter = Optional.ofNullable(rateLimiter).orElse(null);
    }
...
//                        records = consumer.poll(Duration.ofMillis(pollTimeout));
    ...
    protected ConsumerRecords<byte[], byte[]> rateLimitPoll() {

        Map<TopicPartition, List<ConsumerRecord<byte[], byte[]>>> recordsResult = Maps.newHashMap();
        ConsumerRecords<byte[], byte[]> records;
        Lock lock = RaliKafkaConsumerThread.this.lock.readLock();
        lock.lock();
        try {
            records = consumer.poll(Duration.ofMillis(pollTimeout));
        } finally {
        ...
        for (TopicPartition topicPartition : records.partitions()) {
            ...
            if (CollectionUtil.isNullOrEmpty(recordList)) {
                continue;
            }

            PartitionRateLimiter.PermitAcquired permitAcquired = rateLimiter.acquire(topicPartition.topic(), topicPartition.partition(), recordList.size());
            Long nextPollOffset = 0L;
            if (permitAcquired.getAcquired() > 0) {
                if (permitAcquired.getAcquired() < recordList.size()) {
                    //许可数量小于本次拉取数量
                    recordList = recordList.subList(0, (int) permitAcquired.getAcquired());
                    consumer.seek(topicPartition, recordList.get(recordList.size() - 1).offset() + 1);
                }
                recordsResult.put(topicPartition, recordList);
            } else {
                //说明没有获取到许可 获取本次单分区的最早的offset
                nextPollOffset = recordList.get(0).offset();
                consumer.seek(topicPartition, nextPollOffset);
            }

            if (permitAcquired.getWaitNano() > 0) {
                doKafkaPause(topicPartition, permitAcquired);
            }

        }
        return new ConsumerRecords<>(recordsResult);
    }

    protected void doKafkaPause(TopicPartition topicPartition, PartitionRateLimiter.PermitAcquired permitAcquired) {
        log.info("prepare pause kafka consumer!  partition: {}，waitNano: {}", topicPartition.partition(), permitAcquired.getWaitNano());
        try {
            consumer.pause(Arrays.asList(topicPartition));
            timer.schedule(new KafkaResume(topicPartition), (permitAcquired.getWaitNano() <= 1000000) ? 1 : permitAcquired.getWaitNano() / 1000000);
        } catch (Exception e) {
            log.error("topicPartition:{} pause fail! case:{}", topicPartition, e);
        }
    }

    class KafkaResume extends TimerTask {

        private TopicPartition topicPartition;

        public KafkaResume(TopicPartition topicPartition) {
            this.topicPartition = topicPartition;
        }

        @Override
        public void run() {
            Lock lock = RaliKafkaConsumerThread.this.lock.writeLock();
            lock.lock();
            try {
                consumer.resume(Arrays.asList(topicPartition));
                log.info("already resume kafka consumer!  partition: {}", topicPartition.partition());
            } catch (Exception e) {
                log.error("topicPartition:{} resume fail! case:{}", topicPartition, e);
            } finally {
                lock.unlock();
            }
        }
