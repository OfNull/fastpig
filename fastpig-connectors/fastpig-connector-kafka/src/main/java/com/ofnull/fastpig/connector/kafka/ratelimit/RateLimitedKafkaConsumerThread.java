 ...
 */
@Internal
public class RateLimitedKafkaConsumerThread<T> extends Thread {
    ...
    private Map<TopicPartition, WaitState> waitStates;

    public RateLimitedKafkaConsumerThread(
            Logger log,
            Handover handover,
...
//                        records = consumer.poll(Duration.ofMillis(pollTimeout));
    ...
    protected ConsumerRecords<byte[], byte[]> rateLimitPoll() {
        ...
        for (TopicPartition topicPartition : records.partitions()) {
            ...
            if (rateLimiter != null) {
                WaitState waitState = waitStates.get(topicPartition);
                if (waitState != null) {
                    //说明还没有达到可以许可的时间
                    continue;
                }
            ...
            if (permitAcquired.getWaitNano() > 0) {
                WaitState waitState = new WaitState(System.nanoTime() + permitAcquired.getWaitNano(), nextPollOffset);
                waitStates.put(topicPartition, waitState);
            }
    ...
    private void reassign() {
        for (Map.Entry<TopicPartition, WaitState> stateEntry : waitStates.entrySet()) {
            TopicPartition topicPartition = stateEntry.getKey();
            WaitState waitState = stateEntry.getValue();
            if (waitState == null) {
                continue;
            }

            if (waitState.waitUntil < System.nanoTime()) {
                // 等待事件小于 当前系统事件 可以重新拉取数据了
                consumer.seek(topicPartition, waitState.getOffset());
                stateEntry.setValue(null); //清空等待状态
            } else {
                //重置位点到最后 防止没有获取到许可的时候 一直poll
    ...
    public static class WaitState {
        ...
        public WaitState(long waitUntil, long offset) {
